...
func TestManagerDegradeAllFailJoins(t *testing.T) {
	...
	_, err := m.Financials(context.Background(), "600519")
	if err == nil {
		t.Fatal("全失败应返回错误")
	}
	s := err.Error()
...
func TestManagerDegradeAllNotSupported(t *testing.T) {
	...
	_, err := m.Financials(context.Background(), "000001")
...
func TestManagerHKChainUsedForHKCode(t *testing.T) {
	...
	for code, want := range map[string]string{"00700": "20261231", "06198": "20261231", "600519": "20260331"} {
...
func TestManagerDividendDegrade(t *testing.T) {
	...
	v, err := m.DividendPerShare(context.Background(), "600519")
	...
	v, err = m2.DividendPerShare(context.Background(), "600519")
	if !errors.Is(err, ErrNotSupported) {
...
func emHKResp(t *testing.T, rows []map[string]any) string {
...
func TestEMHKFinanceFinancialsEndToEnd(t *testing.T) {
	...
	em.AttachTestTransport(func(r *http.Request) (*http.Response, error) {
		...
		case "RPT_CUSTOM_HKF10_FN_MAININDICATORMAX":
			return jsonHandler(t, emHKResp(t, []map[string]any{max}))(r)
		}
	...
	fx := 0.91
	f, err := src.Financials(context.Background(), "00700", &fx)
	if err != nil {
...
func TestEMHKFinanceNetworkError(t *testing.T) {
	...
	src := NewEMHKFinance(em)
	_, err := src.Financials(context.Background(), "00700", ptrF(0.91))
	if err == nil {
		t.Fatal("网络错误应上抛")
	}
}
...
func TestEMHKFinanceNonHKCode(t *testing.T) {
	em := raw.NewEM()
	src := NewEMHKFinance(em)
	if _, err := src.Financials(context.Background(), "600519", ptrF(0.91)); !errors.Is(err, ErrNotSupported) {
...
func TestEMHKDividendPerShare(t *testing.T) {
	...
	src := NewEMHKFinance(em)
	v, err := src.DividendPerShare(context.Background(), "00700")
...
func TestEMHKDividendPerShareMissing(t *testing.T) {
	em := raw.NewEM()
	em.AttachTestTransport(jsonHandler(t, emHKResp(t, []map[string]any{{"REPORT_DATE": "20260630"}})))
	src := NewEMHKFinance(em)
	if _, err := src.DividendPerShare(context.Background(), "06198"); !errors.Is(err, ErrNotSupported) {
...
func TestNormalizeCNYReporterNoConversion(t *testing.T) {
	...
	rows := []map[string]any{
		{"REPORT_DATE": "2026-06-30 00:00:00", "BPS": 7.30, "BASIC_EPS": 0.6,
			"HOLDER_PROFIT": 3000000000.0, "HOLDER_PROFIT_YOY": 5.0,
			"OPERATE_INCOME": 20000000000.0, "OPERATE_INCOME_YOY": 8.0,
			"ROE_AVG": 10.0, "ROA": 6.0, "EPS_TTM": 0.8, "ROE_YEARLY": 11.0},
		{"REPORT_DATE": "2026-03-31 00:00:00", "BPS": 7.15, "BASIC_EPS": 0.3,
			"HOLDER_PROFIT": 1500000000.0, "HOLDER_PROFIT_YOY": 4.0,
			"OPERATE_INCOME": 10000000000.0, "OPERATE_INCOME_YOY": 6.0,
			"ROE_AVG": 4.0, "ROA": 2.5, "EPS_TTM": 0.8, "ROE_YEARLY": 11.0},
		{"REPORT_DATE": "2025-12-31 00:00:00", "BPS": 7.0, "BASIC_EPS": 1.1,
			"HOLDER_PROFIT": 2000000000.0, "HOLDER_PROFIT_YOY": 6.0,
			"OPERATE_INCOME": 19000000000.0, "OPERATE_INCOME_YOY": 7.0,
			"ROE_AVG": 11.0, "ROA": 6.5, "EPS_TTM": 1.1, "ROE_YEARLY": 11.0},
		{"REPORT_DATE": "2025-06-30 00:00:00", "BPS": 6.8, "BASIC_EPS": 0.55,
			"HOLDER_PROFIT": 1200000000.0, "HOLDER_PROFIT_YOY": 5.0,
			"OPERATE_INCOME": 9500000000.0, "OPERATE_INCOME_YOY": 7.5,
			"ROE_AVG": 5.5, "ROA": 3.0, "EPS_TTM": 1.0, "ROE_YEARLY": 11.0},
	}
	max := map[string]any{
		"REPORT_DATE":          "2026-06-30 00:00:00",
		"ISSUED_COMMON_SHARES": 4800000000.0,
		"TOTAL_MARKET_CAP":     21000000000.0, // 港元市值
		"PE_TTM":               5.03,
		"PB_TTM":               0.546,
		"DIVIDEND_TTM":         0.42,
		"DIVI_RATIO":           38.2,
...
func TestEMHKFinanceNoFxEndToEnd(t *testing.T) {
	...
	em.AttachTestTransport(func(r *http.Request) (*http.Response, error) {
		...
		case "RPT_CUSTOM_HKF10_FN_MAININDICATORMAX":
			return jsonHandler(t, emHKResp(t, []map[string]any{hkMaxRow()}))(r)
		}
		return jsonHandler(t, emHKResp(t, nil))(r)
	})
	src := NewEMHKFinance(em)
	if _, err := src.Financials(context.Background(), "00700", nil); !errors.Is(err, ErrNotSupported) {
...
func TestManagerEMHKWithFx(t *testing.T) {
	...
	f, err := fm.Financials(context.Background(), "00700")
	if err != nil || f == nil {
		t.Fatalf("有汇率应成功，err=%v", err)
	}
	// 无汇率 → 港股链不可用，Manager 返回 ErrNotSupported
	fm2 := NewFinanceManager(nil, nil, []FinanceSource{hk})
	if _, err := fm2.Financials(context.Background(), "00700"); !errors.Is(err, ErrNotSupported) {
...
func TestAshareFinanceSinaError(t *testing.T) {
	...
	if _, err := a.Financials(context.Background(), "600519", nil); err == nil {
		t.Fatal("新浪网错应上抛")
	}
}
...
func TestAshareFinanceHKCodeNotSupported(t *testing.T) {
	...
	if _, err := a.Financials(context.Background(), "00700", ptrF(0.91)); !errors.Is(err, ErrNotSupported) {
...
func TestTTMFromRows(t *testing.T) {
	...
	rows := []map[string]any{
		{"REPORT_DATE": "2026-06-30 00:00:00", "HOLDER_PROFIT": float64(106900000000)},
		{"REPORT_DATE": "2025-12-31 00:00:00", "HOLDER_PROFIT": float64(88000000000)},
		{"REPORT_DATE": "2025-06-30 00:00:00", "HOLDER_PROFIT": float64(43000000000)},
	}
	v := ttmFromRows(rows)
	...
	rows3 := []map[string]any{
		{"REPORT_DATE": "2026-03-31 00:00:00", "HOLDER_PROFIT": float64(26000000000)},
		{"REPORT_DATE": "2025-12-31 00:00:00", "HOLDER_PROFIT": float64(88000000000)},
		{"REPORT_DATE": "2025-03-31 00:00:00", "HOLDER_PROFIT": float64(22000000000)},
	}
	...
	rows2 := []map[string]any{
		{"REPORT_DATE": "2026-06-30 00:00:00", "HOLDER_PROFIT": float64(1)},
	}
...
func TestIsHKCodeAndToSymbol(t *testing.T) {
	if !isHKCode("00700") || !isHKCode("06198") || isHKCode("600519") || isHKCode("000001") || isHKCode("1100abc") {
		t.Fatal("isHKCode 判定错误")
	}
	if toSymbol("600519") != "sh600519" || toSymbol("000001") != "sz000001" {
		t.Fatal("toSymbol 错误")
	}
}
