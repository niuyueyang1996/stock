...
func (s *Service) SetExpectedGrowth(code string, growth float64) {
	...
	s.DB.Exec(`INSERT INTO stock_expected_growth(code, growth, updated_at) VALUES(?,?,?)
	           ON CONFLICT(code) DO UPDATE SET growth=excluded.growth, updated_at=excluded.updated_at`, code, growth, now)
}
...
func (s *Service) SetExpectedRevenueGrowth(code string, growth float64) {
	now := time.Now().Format("2006-01-02T15:04:05")
	s.DB.Exec(`INSERT INTO stock_expected_revenue_growth(code, growth, updated_at) VALUES(?,?,?)
	           ON CONFLICT(code) DO UPDATE SET growth=excluded.growth, updated_at=excluded.updated_at`, code, growth, now)
}
...
func (s *Service) SetExpectedPayout(code string, payout float64) {
	now := time.Now().Format("2006-01-02T15:04:05")
	s.DB.Exec(`INSERT INTO stock_expected_payout(code, payout, updated_at) VALUES(?,?,?)
	           ON CONFLICT(code) DO UPDATE SET payout=excluded.payout, updated_at=excluded.updated_at`, code, payout, now)
}
