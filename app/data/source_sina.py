...
- 资金流：  腾讯分笔 stock.gtimg.cn（并发分页 + 增量快照）
"""
import math
import re
...
def _fetch_ticks(code: str) -> list[tuple[str, float, int]]:
...
class SinaSource(DataSource):
    ...
    def daily_fundflow(self, code: str) -> list:
        """当日五档资金流（腾讯分笔派生，自适应分位分档）。港股/失败返回空。"""
        ticks = _fetch_ticks(code)
        if not ticks:
            return []
    ...
    def fundflow_intraday(self, code: str) -> list:
        """当日分时五档资金流（1 分钟基础粒度，1/5/15/30 由读取侧 resample_points 派生）。"""
        ticks = _fetch_ticks(code)
        if not ticks:
            return []
