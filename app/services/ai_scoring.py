...
_PORTFOLIO_HTML_REQUIREMENT = (
    ...
    "\n7. 输出一张「组合调仓指引」表（本报告核心交付），对组合内每只主要标的（权重前 15 或全部）逐行给出："
    "\n   - 标的（名称+代码）、现价、成本、当前权重（%）；"
    "\n   - 建议操作：加仓/持有/观望/减仓/清仓（与顶层 action、各维结论一致，禁止「视情况而定」式含糊）；"
    "\n   - 关键价格：加仓/买入触发价、减仓/卖出触发价、止损价——优先引用系统提供的现价与技术面支撑/压力位"
    "（technical.key_levels），无数据时给参考区间并标 [AI补充]；港股须注明币种（港元）。"
    "\n   - 目标仓位：当前权重 → 建议目标权重区间（%）；"
    "\n   - 一句话调仓理由（联动估值/资金面/技术面/消息面，注明数据日期）。"
    "\n8. 调仓指引必须可执行、每只标的都要有明确操作与价格锚点，不得含糊；"
    "价格与仓位均为基于现有数据的分析参考，表尾须声明「仅供参考，不构成投资建议」。"
